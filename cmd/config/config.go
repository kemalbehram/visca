...
func SetBip44CoinType(config *sdk.Config) {
	config.SetCoinType(visca.Bip44CoinType)
	config.SetPurpose(sdk.Purpose)                  // Shared
	config.SetFullFundraiserPath(visca.BIP44HDPath) //nolint: staticcheck
}
...
func RegisterDenoms() {
	...
	if err := sdk.RegisterDenom(visca.AttoPhoton, sdk.NewDecWithPrec(1, visca.BaseDenomUnit)); err != nil {
		panic(err)
	}
}
