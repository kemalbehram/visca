...
from .network import Visca, setup_custom_visca
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
            ...
            ini[section].update(
                {
                    ...
                    "environment": (
                        f"DAEMON_NAME=viscad,DAEMON_HOME=%(here)s/node{i}"
                    ),
                }
            )
...
def custom_visca(tmp_path_factory):
    ...
    yield from setup_custom_visca(
        ...
        post_init=post_init,
        chain_binary=str(path / "upgrades/genesis/bin/viscad"),
    )


def test_cosmovisor_upgrade(custom_visca: Visca):
    """
    ...
    assert rsp["code"] == 0, rsp["raw_log"]
    # rsp = custom_visca.cosmos_cli(1).gov_vote("validator", proposal_id, "yes")
    # assert rsp["code"] == 0, rsp["raw_log"]
    ...
    custom_visca.chain_binary = (
        Path(custom_visca.chain_binary).parent.parent.parent
        / f"{plan_name}/bin/viscad"
    )
    cli = custom_visca.cosmos_cli()

    # block should pass the target height
    wait_for_block(cli, target_height + 1, timeout=480)
    wait_for_port(ports.rpc_port(custom_visca.base_port(0)))

    # test migrate keystore
    cli.migrate_keystore()

    # check basic tx works after upgrade
    wait_for_port(ports.evmrpc_port(custom_visca.base_port(0)))
