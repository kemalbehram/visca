...
from .network import setup_custom_visca
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:visca_9000-1-node{i}":
            ini[section].update(
                {
...
def custom_visca(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-viscad.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/viscad"
    )
    print(broken_binary)
...
def test_rollback(custom_visca):
    """
    ...
    print("stop node1")
    supervisorctl(
        custom_visca.base_dir / "../tasks.ini", "stop", "visca_9000-1-node1"
    )
    ...
    update_node_cmd(custom_visca.base_dir, "viscad", 1)
    supervisorctl(custom_visca.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
