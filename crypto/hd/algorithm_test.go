...
import (
	...
	enccodec "github.com/onchainengineer/visca/encoding/codec"
...
func TestKeyring(t *testing.T) {
	...
	kr, err := keyring.New("visca", keyring.BackendTest, dir, mockIn, TestCodec, EthSecp256k1Option())
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, visca.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, visca.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := hdwallet.MustParseDerivationPath(visca.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Visca implementation
	...
	// Equality of Eth and Visca implementation
	...
	// Inequality of wrong derivation path of Eth and Visca implementation
